...
Функция `buildSwapTransaction` используется для получения неподписанной транзакции для обмена между двумя токенами.

Перед использованием этой функции убедитесь, что у вас есть [Client API Key](https://portal.cdp.coinbase.com/projects/api-keys/client-key) от Coinbase Developer Platform.
