...
Компонент `Address` используется для отображения сокращенного адреса пользовательского аккаунта.

## Использование

Сокращенный адрес аккаунта:
...
Установите `isSliced` в `false`, чтобы отобразить полный адрес:
